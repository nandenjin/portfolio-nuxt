...
export async function copyAssets(src: string, dist: string): Promise<void> {
      ...
      if (ent.isDirectory()) {
        ...
        if (ent.name.match(/^(.+)\.(jpg|png|gif|webp)$/)) {
          ...
          for (const size of SIZES) {
            const jpgDistPathWithSize = path.join(
              dist,
              `${RegExp.$1}_${size}w.jpg`
            )
            const webpDistPathWithSize = path.join(
              dist,
              `${RegExp.$1}_${size}w.webp`
            )
